        ...
        return (
          <Card key={drill.drillId}>
            <CardHeader>
              ...
              <CardDescription>Tight clusters suggest consistency. Wide spread suggests variability.</CardDescription>
            </CardHeader>
            <CardContent className="h-[320px] px-2 sm:px-6">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data} margin={{ top: 12, right: 20, left: 4, bottom: 20 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)" />
                  <XAxis dataKey="bucket" angle={-25} textAnchor="end" height={60} interval="preserveStartEnd" />
                  ...
                  <ReferenceLine
                    ...
                    stroke="#dc2626"
                    strokeDasharray="6 6"
                    label={{ value: "Standard", fill: "#dc2626", fontSize: 12, position: "insideTopRight" }}
                  />
...
function findReferenceBucket(data: Array<{ bucket: string; mid: number }>, timeStandard: number): string | undefined {
