...
export const trainingSessions = pgTable("training_sessions", {
  ...
  createdBy: integer("created_by").references(() => shooters.id).notNull(),
...
export const sessionShooters = pgTable("session_shooters", {
  ...
  shooterId: integer("shooter_id").references(() => shooters.id).notNull(),
...
export const sessionDrills = pgTable("session_drills", {
  id: serial("id").primaryKey(),
  sessionId: integer("session_id").references(() => trainingSessions.id).notNull(),
  drillId: integer("drill_id").references(() => drills.id).notNull(),
  position: integer("position").notNull(),
});
